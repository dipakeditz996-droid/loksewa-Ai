...
export default function ChapterDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadData = async () => {
    try {
      ...
      const [chapData, topicData] = await Promise.all([
        adminAcademicApi.getChapter(chapterId),
        adminAcademicApi.getTopics(chapterId),
      ]);
      setChapter(chapData);
      setTopics(topicData);
    } catch (error) {
  ...
  const handleOpenModal = (topic?: ApiTopic) => {
    if (topic) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingTopic) {
        await adminAcademicApi.updateTopic(editingTopic.id, formData);
      } else {
        await adminAcademicApi.createTopic({ ...formData, chapter: chapterId, order: topics.length });
      }
  ...
  if (!chapter) {
    return (
      <div className="p-6">
        ...
        <Link href="/admin-dashboard/academic/chapters" className="text-sm text-[#0B2545] underline mt-4 inline-block">
          Back to Chapters
        </Link>
      </div>
    );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <div className="flex items-center gap-2 text-sm text-slate-500 mb-2">
            ...
            <Link href={`/admin-dashboard/academic/subjects/${chapter.subject}`} className="hover:text-[#0B2545] transition-colors">
