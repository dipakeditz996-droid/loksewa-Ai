...
import { Plus, Search, ChevronRight, Edit, Trash2, BookOpen, CheckCircle2, XCircle, Loader2, AlertCircle } from "lucide-react";
import { adminAcademicApi, ApiSubject, ApiChapter } from "@/lib/api/admin-academic-api";
...
export default function SubjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [editingChapter, setEditingChapter] = useState<ApiChapter | null>(null);
  ...
  const loadData = async () => {
    try {
      ...
      const [subData, chapData] = await Promise.all([
        adminAcademicApi.getSubject(subjectId),
        adminAcademicApi.getChapters(subjectId),
      ]);
      setSubject(subData);
  ...
  const handleOpenModal = (chapter?: ApiChapter) => {
    if (chapter) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingChapter) {
        await adminAcademicApi.updateChapter(editingChapter.id, formData);
      } else {
        await adminAcademicApi.createChapter({ ...formData, subject: subjectId, order: chapters.length });
      }
  ...
  const handleDelete = async (id: number, topicCount: number) => {
    ...
    if (confirm("Are you sure you want to delete this chapter?")) {
      try {
        await adminAcademicApi.deleteChapter(id);
        loadData();
      } catch (error) {
  ...
  if (!subject) {
    return (
      <div className="p-6">
        ...
        <Link href="/admin-dashboard/academic/subjects" className="text-sm text-[#0B2545] underline mt-4 inline-block">
          Back to Subjects
        </Link>
      </div>
    );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <div className="flex items-center gap-2 text-sm text-slate-500 mb-2">
            ...
            <Link href={`/admin-dashboard/academic/papers/${subject.paper}`} className="hover:text-[#0B2545] transition-colors">
              {subject.paper_name || "Paper"}
            </Link>
            <ChevronRight className="h-3.5 w-3.5" />
            <span>Chapters</span>
          </div>
                  ...
                  <tr key={chapter.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Link
                          href={`/admin-dashboard/academic/chapters/${chapter.id}`}
