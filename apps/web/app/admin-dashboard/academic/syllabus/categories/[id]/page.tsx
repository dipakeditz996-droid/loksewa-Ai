...
import { Plus, Search, ChevronRight, Edit, Trash2, ArrowUp, ArrowDown, LayoutTemplate, CheckCircle2, XCircle, ArrowLeft, Loader2, AlertCircle } from "lucide-react";
import { adminSyllabusApi, AdminPosition, AdminExamCategory } from "@/lib/api/admin-syllabus";
...
export default function PositionsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [category, setCategory] = useState<AdminExamCategory | null>(null);
  ...
  const loadData = async () => {
    try {
      ...
      const [catData, posData] = await Promise.all([
        adminSyllabusApi.getCategory(categoryId),
  ...
  const handleOpenModal = (position?: AdminPosition) => {
    if (position) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingPosition) {
        await adminSyllabusApi.updatePosition(editingPosition.id, formData);
      } else {
        await adminSyllabusApi.createPosition({ ...formData, category: categoryId, order: positions.length });
      }
  ...
  const handleDelete = async (id: number, subjectCount: number) => {
    if (subjectCount > 0) {
      alert("Cannot delete a position that contains subjects. Please delete or move the subjects first.");
      return;
    }
    if (confirm("Are you sure you want to delete this position?")) {
      try {
        await adminSyllabusApi.deletePosition(id);
        loadData();
      } catch (error) {
  ...
  const handleMove = async (index: number, direction: 'up' | 'down') => {
    ...
    const target = newPositions[targetIndex];
    ...
    newPositions[index] = target;
    newPositions[targetIndex] = current;

    setPositions(newPositions);

    try {
      await adminSyllabusApi.reorderPositions([
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-[#0B2545] flex items-center gap-2">
            ...
            Positions in <span className="text-[#D4A72C]">{category.name}</span>
          </h1>
          <p className="text-slate-500 mt-1">Manage positions/levels for this category.</p>
        </div>
        <button 
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col sm:flex-row gap-4 items-center justify-between">
        ...
        <div className="text-sm text-slate-500">
          Showing <span className="font-medium text-[#0B2545]">{filteredPositions.length}</span> positions
        </div>
      </div>
                  ...
                  <tr key={position.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1">
                        ...
                        <button 
                          disabled={index === positions.length - 1 || search !== ""}
                    ...
                    <td className="px-6 py-4">
                      <div className="font-semibold text-[#0B2545] text-base">{position.name}</div>
                      {position.description && (
                        <div className="text-slate-500 mt-1 line-clamp-1">{position.description}</div>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center justify-center px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-600 font-medium text-xs">
                        {position.subject_count} subjects
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      {position.is_active ? (
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => handleDelete(position.id, position.subject_count)}
                        ...
                        <Link
                          href={`/admin-dashboard/academic/syllabus/positions/${position.id}`}
            ...
            <form onSubmit={handleSubmit}>
              <div className="p-6 space-y-4">
                <div className="space-y-1.5">
                  <label className="text-sm font-semibold text-slate-700">Position Name <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                    required
                    value={formData.name}
                    onChange={(e) => setFormData({...formData, name: e.target.value})}
                    placeholder="e.g. Section Officer, Nayab Subba"
