...
export default function ExamsLayout({ children }: { children: React.ReactNode }) {
  ...
  // If the path includes an ID (and isn't 'new' or 'categories'), we might not want to show the top nav, or we highlight nothing.
  // Actually, we'll keep the top nav, but it won't be active if we are deep inside a specific exam.
  const isDeepRoute = pathname.split("/").length > 3 && !pathname.includes("/new") && !pathname.includes("/categories");

  const isActive = (href: string, exact?: boolean) => {
    if (exact) return pathname === href;
    // Don't highlight "Overview" if we are in a deep route like /exams/[id]
    if (href === "/admin-dashboard/exams" && isDeepRoute) return false;
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-8">
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1">Create, manage and analyze competitive examination tests.</p>
            </div>
