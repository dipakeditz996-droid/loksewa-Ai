    ...
    <div className="p-4 lg:p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-[800] text-primary dark:text-foreground tracking-tight">My Courses</h1>
          <p className="text-muted-foreground mt-1 font-medium max-w-2xl">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((i) => (
            <div key={i} className="bg-card rounded-[24px] border border-border shadow-sm p-5 animate-pulse">
              <div className="h-40 bg-muted/80 rounded-[16px] mb-4"></div>
              <div className="h-6 bg-muted/80 rounded mb-2 w-3/4"></div>
              <div className="h-4 bg-muted/80 rounded mb-4 w-full"></div>
              <div className="space-y-2 mb-4">
                <div className="h-2 bg-muted/80 rounded w-full"></div>
                <div className="h-4 bg-muted/80 rounded w-1/4"></div>
              </div>
              <div className="h-10 bg-muted/80 rounded-[12px] w-full"></div>
            </div>
          ))}
        </div>
      ) : error ? (
        <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/50 text-red-700 dark:text-red-300 p-8 rounded-[20px] text-center">
        ...
        <div className="bg-card border border-border shadow-sm rounded-[24px] p-12 text-center flex flex-col items-center justify-center min-h-[400px]">
          <div className="w-20 h-20 bg-muted rounded-full flex items-center justify-center mb-4">
            <BookMarked className="w-10 h-10 text-slate-300" />
          </div>
          <h3 className="text-xl font-bold text-primary dark:text-foreground mb-2">No active courses</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {courses.map((item) => (
            <div 
              ...
              className="bg-card rounded-[24px] border border-border shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group flex flex-col"
            >
              <div className="relative h-48 bg-muted/80 overflow-hidden">
                ...
                <div className="absolute top-3 left-3 flex gap-2">
                  {item.course.exam && (
                    <span className="bg-card/90 backdrop-blur-sm text-primary dark:text-foreground text-[10px] font-bold px-2.5 py-1 rounded-full shadow-sm">
                ...
                <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <Link href={`/student/courses/${item.course.id}`} className="w-12 h-12 bg-card/20 backdrop-blur-md rounded-full flex items-center justify-center hover:bg-card/30 transition-colors text-white">
              ...
              <div className="p-5 flex-1 flex flex-col">
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-primary dark:text-foreground leading-tight mb-2 line-clamp-2 group-hover:text-blue-600 transition-colors">
                ...
                <div className="mt-auto space-y-4 pt-4 border-t border-border/50">
                  <div>
                    <div className="flex justify-between items-end mb-1.5">
                      ...
                      <span className="text-xs font-bold text-primary dark:text-foreground">{item.progress.percentage}%</span>
                    </div>
                    <Progress 
                    ...
                    />
                    <div className="flex justify-between items-center mt-1.5 text-[10px] font-medium text-muted-foreground">
                  ...
                  <Button asChild className="w-full bg-primary text-primary-foreground hover:bg-[#163E6B] text-white font-semibold rounded-[12px] h-10 shadow-sm transition-all group-hover:shadow-md">
                    <Link href={`/student/courses/${item.course.id}`}>
