...
export default function GameHistoryPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8 min-h-[calc(100vh-72px)] bg-muted/50">
      <div>
        <h1 className="text-3xl font-bold text-primary dark:text-foreground">Games History & Rankings</h1>
      ...
      <Tabs defaultValue="history">
        ...
        <TabsContent value="history" className="mt-6 space-y-8">
          {/* 1v1 History */}
          <div>
            <h2 className="text-xl font-bold flex items-center mb-4 text-primary dark:text-foreground">
              <Swords className="w-5 h-5 mr-2 text-primary dark:text-foreground" /> Recent 1 vs 1 Matches
            </h2>
                ...
                <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {history?.survivals.map(surv => (
                      <tr key={surv.id} className="hover:bg-muted">
                        <td className="px-6 py-4 text-muted-foreground">
                        ...
                        <td className="px-6 py-4 font-bold text-primary dark:text-foreground">
                          {surv.score}
                        </td>
                        <td className="px-6 py-4">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="bg-card border rounded-xl p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-primary dark:text-foreground flex items-center">
                  <Swords className="w-5 h-5 mr-2 text-primary dark:text-foreground" /> Top Duelists
                </h2>
                <span className="text-xs bg-muted/80 px-2 py-1 rounded text-muted-foreground font-medium">Most Wins</span>
              </div>
              ...
              {leaderboard?.top_1v1.length === 0 ? (
                ...
                <div className="space-y-4">
                  {leaderboard?.top_1v1.map((profile, idx) => (
                    <div key={profile.id} className="flex items-center justify-between p-4 rounded-lg bg-muted border border-border/50">
                      <div className="flex items-center space-x-4">
                        <div className={`
                          w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm
                          ${idx === 0 ? 'bg-[#D4A72C] text-white' : 
                            idx === 1 ? 'bg-secondary text-foreground' : 
                            idx === 2 ? 'bg-amber-700 text-white' : 'bg-muted/80 text-muted-foreground'}
                        `}>
                          {idx + 1}
                        </div>
                        <span className="font-semibold text-primary dark:text-foreground">{profile.username}</span>
                      </div>
                      <div className="font-bold text-primary dark:text-foreground">
                        {profile.total_1v1_wins} <span className="text-xs font-normal text-muted-foreground ml-1">Wins</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-card border rounded-xl p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-xs bg-muted/80 px-2 py-1 rounded text-muted-foreground font-medium">Highest Score</span>
              </div>
                  ...
                  {leaderboard?.top_survival.map((profile, idx) => (
                    <div key={profile.id} className="flex items-center justify-between p-4 rounded-lg bg-muted border border-border/50">
                      ...
                      <div className="text-right">
                        <div className="font-bold text-primary dark:text-foreground">{profile.best_survival_score}</div>
                        <div className="text-xs text-muted-foreground">Best Streak: {profile.best_survival_streak}</div>
                      </div>
                    </div>
                  ))}
                </div>
