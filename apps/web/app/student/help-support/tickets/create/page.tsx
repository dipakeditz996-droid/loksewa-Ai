  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-primary dark:text-foreground">Report a Problem</h1>
        <p className="text-sm text-muted-foreground mt-1">Provide details about your issue so we can help you quickly.</p>
      </div>

      <div className="bg-card rounded-2xl border border-border/80 shadow-sm p-6 md:p-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            <Label className="text-[13px] font-semibold text-foreground">Subject <span className="text-red-500">*</span></Label>
            <Input
              value={form.subject}
              onChange={(e) => setForm({ ...form, subject: e.target.value })}
              placeholder="Brief summary of the issue..."
              className="bg-muted border-border focus:border-[#D4A72C] focus:ring-[#D4A72C]/20"
              required
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="space-y-2">
              <Label className="text-[13px] font-semibold text-foreground">Category <span className="text-red-500">*</span></Label>
              <select
                value={form.category}
                onChange={(e) => setForm({ ...form, category: e.target.value })}
                className="w-full bg-muted border border-border rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-[#D4A72C]/20 focus:border-[#D4A72C]"
              >
                {CATEGORIES.map(c => <option key={c.key} value={c.key}>{c.label}</option>)}
              </select>
            </div>
            <div className="space-y-2">
              <Label className="text-[13px] font-semibold text-foreground">Priority <span className="text-red-500">*</span></Label>
              <select
                value={form.priority}
          ...
          <div className="space-y-2">
            <Label className="text-[13px] font-semibold text-foreground">Description <span className="text-red-500">*</span></Label>
            <Textarea
              ...
              className="min-h-[150px] bg-muted border-border focus:border-[#D4A72C] focus:ring-[#D4A72C]/20 resize-y"
              required
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t border-border/50">
            <div className="space-y-2">
              <Label className="text-[13px] font-semibold text-foreground">Related Exam (Optional)</Label>
              <Input
            ...
            <div className="space-y-2">
              <Label className="text-[13px] font-semibold text-foreground">Question Number (Optional)</Label>
              <Input
          ...
          <div className="pt-6 flex justify-end">
            <Button
              ...
              className="bg-primary text-primary-foreground hover:bg-[#163E6B] text-white px-8"
            >
              {mutation.isPending ? (
