    ...
    <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
      ...
      <div
        className={`absolute top-0 left-0 w-full h-1 ${
          isPending ? "bg-amber-400" : passed ? "bg-emerald-500" : "bg-rose-500"
        }`}
      />
        ...
        <div>
          ...
          {r.score !== null ? (
            <div className="flex items-baseline gap-1 mt-0.5">
              ...
              {r.percentage !== null && (
                <span className="text-sm font-bold text-slate-400">({pct.toFixed(1)}%)</span>
              )}
            </div>
          ) : (
            <span className="text-sm font-medium text-slate-400 mt-0.5 block">Pending</span>
          )}
        </div>
...
export default function StudentResultsPage() {
  ...
  const fetchResults = useCallback(async () => {
    ...
    setError(false);
    try {
      const res = await evaluationService.getMyResults({ page, page_size: PAGE_SIZE });
      // Only show submitted or evaluated attempts (not in-progress)
      const finished = res.results.filter((a) => a.status !== "in-progress");
