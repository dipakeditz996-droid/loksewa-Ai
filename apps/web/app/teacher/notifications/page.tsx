...
import { Check, CircleAlert, FileText, HelpCircle, Users, Activity, Search, Filter } from 'lucide-react';
...
export default function NotificationsPage() {
  ...
  const fetchNotifications = async () => {
    setIsLoading(true);
    try {
      // In a real app, you'd pass filter params to the API. For now we fetch all and filter client-side or use basic API filters.
  ...
  return (
    <div className="flex-1 p-4 md:p-8 max-w-5xl mx-auto w-full">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-[#0B2545]">Notifications</h1>
          <p className="text-slate-500 mt-1">Stay updated on your students, content reviews, and teaching activity.</p>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="outline" className="text-slate-600" onClick={() => router.push('/teacher/settings?tab=notifications')}>
            Settings
          </Button>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sticky top-24">
            <div className="relative mb-4">
              ...
              <Input 
                ...
                className="pl-9 bg-slate-50 border-slate-200 text-sm h-10 rounded-xl focus-visible:ring-[#D4A72C]"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              />
            </div>
            
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 px-3 mt-6">Filters</h3>
              ...
              {[
                ...
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                    filter === f.id ? 'bg-[#0B2545]/5 text-[#0B2545]' : 'text-slate-600 hover:bg-slate-50'
                  }`}
                >
                  {f.label}
                </button>
        ...
        <div className="flex-1">
          {isLoading ? (
            <div className="space-y-4">
              {[1, 2, 3, 4].map(i => (
                <div key={i} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 h-28">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-12 text-center flex flex-col items-center">
              ...
              <h2 className="text-xl font-bold text-[#0B2545] mb-2">You're all caught up</h2>
              ...
              {filteredNotifications.map((notif) => (
                <div 
                  ...
                  className={`bg-white rounded-2xl border transition-all cursor-pointer p-5 flex items-start gap-4 ${
                    !notif.is_read 
                      ? 'border-indigo-100 shadow-sm bg-indigo-50/20' 
                      : 'border-slate-200 hover:border-slate-300 hover:shadow-sm opacity-80'
                  }`}
                >
                  <div className={`mt-1 flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${
                    !notif.is_read ? 'bg-white shadow-sm border border-slate-100' : 'bg-slate-50'
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start gap-2 mb-1">
                      <h3 className={`text-base truncate pr-4 ${!notif.is_read ? 'font-bold text-[#0B2545]' : 'font-semibold text-slate-700'}`}>
                        {notif.title}
                      </h3>
                    ...
                    {notif.action_url && (
                      <div className="mt-3">
                        <span className="text-sm font-semibold text-[#D4A72C] hover:text-[#0B2545] transition-colors">
                          View details &rarr;
                        </span>
                      </div>
                    )}
