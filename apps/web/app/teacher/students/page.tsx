...
export default function TeacherStudentsPage() {
  ...
  const selectClass = "h-9 rounded-lg border border-border bg-card px-3 text-[12.5px] font-medium text-foreground focus:outline-none focus:ring-2 focus:ring-[#0B2545]/20";
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4 md:p-8">
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-[0_1px_2px_rgba(16,24,40,0.04)]">
        ...
        {/* Toolbar */}
        <div className="flex flex-col gap-4 border-b border-border bg-muted p-4">
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            ...
            <table className="w-full min-w-[840px] border-collapse text-left">
              <thead>
                <tr>
                  <th className="border-b border-border px-5 py-3 text-[11px] font-bold uppercase tracking-[0.04em] text-muted-foreground">Student</th>
                  <th className="border-b border-border px-5 py-3 text-[11px] font-bold uppercase tracking-[0.04em] text-muted-foreground">Courses</th>
                  <th className="border-b border-border px-5 py-3 text-[11px] font-bold uppercase tracking-[0.04em] text-muted-foreground">Performance</th>
                  <th className="border-b border-border px-5 py-3 text-[11px] font-bold uppercase tracking-[0.04em] text-muted-foreground">Status & Activity</th>
              ...
              <tbody>
                {filteredStudents.map((student) => (
                  <tr key={student.id} className="group hover:bg-muted">
                    <td className="border-b border-[#F2F4F8] px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full border border-border bg-primary/10">
                          {student.avatar ? (
                            <RetryImage src={student.avatar} alt={student.first_name} className="h-full w-full object-cover" />
                          ) : (
                        ...
                        <div>
                          ...
                          <div className="text-[12px] text-muted-foreground">{student.email}</div>
                        </div>
                      </div>
                    </td>
                    <td className="border-b border-[#F2F4F8] px-5 py-4">
                      <div className="flex items-center gap-2 text-[13px]">
                        ...
                        <span className="font-semibold text-foreground">{student.enrolled_courses}</span>
                        <span className="text-muted-foreground">enrolled</span>
                      </div>
                    </td>
                    <td className="border-b border-[#F2F4F8] px-5 py-4">
                      <div className="flex flex-col gap-1.5">
                        <div className="text-[13px] font-semibold text-foreground">Avg: {student.average_score.toFixed(1)}%</div>
                    ...
                    <td className="border-b border-[#F2F4F8] px-5 py-4">
                      <div className="flex flex-col gap-1.5">
                        <span className={cn(
                          ...
                          student.status === 'Active' ? "bg-[#0F7A69]/10 text-[#0F7A69] dark:text-[#4ADE9C]" :
                          student.status === 'Inactive' ? "bg-[#EEF1F6] text-muted-foreground" :
