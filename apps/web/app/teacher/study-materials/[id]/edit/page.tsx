...
import { ArrowLeft, Upload, File as FileIcon, X, AlertTriangle } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Textarea } from '@/components/ui/textarea';
import { toast } from 'sonner';
import { getTeacherMaterial, updateTeacherMaterial } from '@/lib/api/teacher-materials';
...
export default function EditStudyMaterialPage() {
  ...
  useEffect(() => {
    ...
    // Fetch Material
    getTeacherMaterial(materialId).then(data => {
      setFormData({
        title: data.title || '',
        description: data.description || '',
        material_type: data.material_type || 'notes',
      ...
      setExistingFileUrl(data.file || null);
  ...
  const handleSubmit = async (submitType: 'save' | 'submit') => {
    ...
    try {
      ...
      Object.keys(formData).forEach(key => {
        if (formData[key as keyof typeof formData] !== null && formData[key as keyof typeof formData] !== undefined) {
      ...
      const res = await updateTeacherMaterial(materialId, submitData);

      if (submitType === 'submit') {
        await apiClient(`/notes/teacher/materials/${materialId}/submit/`, { method: 'POST' });
