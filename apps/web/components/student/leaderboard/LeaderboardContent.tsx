...
function AnalyticsPanel({ stats, trend, loading, topStudents = [] }: AnalyticsPanelProps) {
        ...
        <div className="lb-glass rounded-2xl p-5">
          ...
          <div className="space-y-3">
            {[
              ...
              .filter(({ rank }) => Boolean(topStudents[rank - 1]))
              .map(({ rank, icon, color }) => {
                const student = topStudents[rank - 1];
                ...
                return (
                  <div key={rank} className="flex items-center gap-3">
                    ...
                    <Avatar className="w-7 h-7 flex-shrink-0">
                      {student.avatar && <AvatarImage src={student.avatar} alt={student.studentName} />}
                      <AvatarFallback
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-white text-xs font-semibold truncate">
                        ...
                        {student.isCurrentUser && (
                          <span className="ml-1 text-[8px] text-yellow-400 font-bold">YOU</span>
                        )}
                      </p>
                      <p className="text-white/40 text-[10px]">{student.score}/{student.maxScore}</p>
                    </div>
                    <span className="text-[10px] font-bold flex-shrink-0" style={{ color }}>
                      {fmt(student.percentage)}%
                    </span>
                  </div>
                );
              })}
